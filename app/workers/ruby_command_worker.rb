...
class RubyCommandWorker < BackgrounDRb::Rails
...
begin
      while true
        ...
        puts "after wait for evaluate now" 
        ...
        str_result = nil
        @current_command = RubyCommand.new(:command => @evaluate_command, 
...
puts "before inspect parser"
            str_result = InspectParser.new(inspect_str, 
              RubyCommandHelper::ActiveRecordProxyConstructor.new).parse.inspect
...
puts "before @current_command.save"
	...
	puts "Thread Exception: #{e.message}"
